        ...
        <div class="col-lg-6">
          <div class="card ">
            <div class="card-header">
              <h3 class="text-center title-2">View Menu Form</h3>
              @if (Session::has('pharmacy_msg'))
              {!! session('pharmacy_msg') !!}
              @endif
            </div>
            <div class="card-body">
              <form action="/menu/update/{{ $edit['id'] }}" method="POST">
                @csrf
                <div class="form-group">
                  <label for="name" class="control-label mb-1">EName</label>
                ...
                <div class="form-group">
                  ...
                  <input id="link" name="link" type="text" class="form-control" value="{{ $edit['link'] }}" required />
                </div>
                <div class="form-group">
                  ...
                  <select id="menu_group_id" name="menu_group_id" class="form-control" required>
                    ...
                    @if($menugroup['id'] == $edit['menu_group_id'])
                    <option value="{{ $menugroup['id'] }}" selected>{{ $menugroup['name'] }}</option>
                    @else
                ...
                <div>
                  <button type="submit" class="btn btn-lg btn-dark btn-block">
                    <i class="fa fa-save fa-lg"></i> Update Menu
                  </button>
                </div>
              </form>
            </div>
